...
{
	@FXML Button missedAppointmentButton;
	...
	private int attendanceTrackNum;
	...
	public void addMissedAppointment()
	{
		boolean checkIfAnythingIsSelected = patientListTableView.getSelectionModel().isEmpty();
		if (!checkIfAnythingIsSelected)
		{
			TablePosition pos = patientListTableView.getSelectionModel().getSelectedCells().get(0);
			int row = pos.getRow();
			Patient patient = patientListTableView.getItems().get(row);
			patientIDNumber = patient.getIdNumberColumn();
			ResultSet patientResultSetTrack = Adapter.getResultSet("patient where idNumber='"+patientIDNumber+"'");
			try
			{
				while (patientResultSetTrack.next())
				{
			attendanceTrackNum = patientResultSetTrack.getInt("attendanceTrack");
			String patientFN = patientResultSetTrack.getString("firstName");
			String patientLN = patientResultSetTrack.getString("lastName");
			attendanceTrackNum++;
			String attendanceTrackStr = Integer.toString(attendanceTrackNum);
			Patient.updateSpecificColumn(patientIDNumber,"attendanceTrack", attendanceTrackStr);
			missedAppointmentButton.setOpacity(0.5);
			missedAppointmentButton.setText("Added missed appointment to " + patientFN+" "+patientLN);
		}
				}
				catch (SQLException e) 
		{
